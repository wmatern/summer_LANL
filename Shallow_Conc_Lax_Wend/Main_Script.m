...
for t = 0:dt:tmax
    ...
    for i = 3:length(X(:,1))+2
        for j = 3:length(Y(1,:))+2
            Hxminus = U_halfstep(dt, Hl, Hic, HXFLUXNL, HXFLUXIC, dxl,...
                dxic, dxl, dxic, sqrt(dxl), sqrt(dxic), V_i, V_n);
            Uxminus = U_halfstep(dt, Ul, Uic, UXFLUXNL, UXFLUXIC,...
                dxl, dxic, dxl, dxic, SQR(dxl), SQR(dxic));
            Vxminus = U_halfstep(dt, Vl, Vic, UVFLUXNL, UVFLUXIC,...
                dxl, dxic, dxl, dxic, SQR(dxl), SQR(dxic));
