...
nstep     = 0;
init_phi = phi;
V_d_p = zeros(nx+4,ny+4);
V_d_m = zeros(nx+4,ny+4);
...
while nstep < 10000000
    ...
    elseif SCH == 2
        ...
        if BC == 2
            ...
            V_d_p = Periodic_BCy(V_d_p,ny);
            
            V_d_m = Periodic_BCx(V_d_m,nx);
...
%             beta_dn_y(phi,phi_New,V_d_m,i,j,dt,dx))).*my_minus(V_d_m(i,j));
    ...
    if cond == 2
        if nstep*dt*max(max(V))/ny == 1 || nstep*dt*max(max(V))/ny == 5 ...
            ...
            phi_analyt = reshape(phi_analyt,nx,ny);
            figure(nstep*dt*max(max(V))/ny),plot(phi(floor(nx/2+2),3:ny+2),'.-') , title('phi'), hold on
