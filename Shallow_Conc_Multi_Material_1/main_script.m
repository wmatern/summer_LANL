...
%        end
       ...
       nu        = (abs(Ux(i-1,j-1)./Hx(i-1,j-1)) + sqrt(g*Hx(i-1,j-1)))*dt/dx;
       cv        = nu.*(1-nu);
       wminusx   = .5*cv.*(1-q);
       ...
       rminus    = rnumminus./rdenom;
       q         = max(min(1.0, min(rminus, rplus)),0.0);
       nu        = (abs(Ux(i,j-1)./Hx(i,j-1)) + sqrt(g*Hx(i,j-1)))*dt/dx;
       cv        = nu.*(1-nu);
       wplusx    = .5*cv.*(1-q);       
       ...
       rminus    = rnumminus./rdenom;
       q         = max(min(1.0, min(rminus, rplus)),0.0);
       nu        = (abs(Vy(i-1,j-1)./Hy(i-1,j-1)) + sqrt(g*Hy(i-1,j-1)))*dt/dy;
       cv        = nu.*(1-nu);
       wminusy   = .5*cv.*(1-q);
       ...
       rminus    = rnumminus./rdenom;
       q         = max(min(1.0, min(rminus, rplus)),0.0);
       nu        = (abs(Vy(i-1,j)./Hy(i-1,j)) + sqrt(g*Hy(i-1,j)))*dt/dy;
       cv        = nu.*(1-nu);
       wplusy    = .5*cv.*(1-q);
       
       % Minmod Flux Limiting
%        duminus2   = phi(i-1,j,k) - phi(i-2,j,k;
%        duplus2    = phi(i+1,j,k) - phi(i  ,j,k);
%        duhalf2    = phi(i  ,j,k) - phi(i-1,j,k);
...
%        duplus2   = phi(i+2,j,k) - phi(i+1,j,k);
%        duhalf2   = phi(i+1,j,k) - phi(i  ,j,k);
%        rdenom    = max(duhalf2.^2, eps);
%        rnumplus  = duplus2 .*duhalf2;
%        rnumminus = duminus2.*duhalf2;
%        rplus     = rnumplus./rdenom;
...
%        wplusphix = .5*cv.*(1-q);       
%        
%        duminus2   = phi(i,j-1,k) - phi(i,j-2,k);
%        duplus2    = phi(i,j+1,k) - phi(i,j  ,k);
%        duhalf2    = phi(i,j  ,k) - phi(i,j-1,k);
%        rdenom     = max(duhalf2.^2, eps);
%        rnumplus   = duplus2 .*duhalf2;
%        rnumminus  = duminus2.*duhalf2;
%        rplus      = rnumplus./rdenom;
...
%        wminusphiy = .5*cv.*(1-q);
%        
%        duminus2  = phi(i,j  ,k) - phi(i,j-1,k);
%        duplus2   = phi(i,j+2,k) - phi(i,j+1,k);
%        duhalf2   = phi(i,j+1,k) - phi(i,j  ,k);
%        rdenom    = max(duhalf2.^2, eps);
%        rnumplus  = duplus2.*duhalf2;
%        rnumminus = duminus2.*duhalf2;
%        rplus     = rnumplus./rdenom;
...
%        wplusphiy = .5*cv.*(1-q);
       ...
       phi (i,j,k) = phi (i,j,k) - (dt/(2*dx))*(Ux (i,j-1)./Hx(i,j-1)...
           ...
           - wminusphix(:,:,k).*(phi(i,j,k) - phi(i-1,j,k)) + wplusphix(:,:,k).*(phi(i+1,j,k)-phi(i,j,k))...
           - wminusphiy(:,:,k).*(phi(i,j,k) - phi(i,j-1,k)) + wplusphiy(:,:,k).*(phi(i,j+1,k)-phi(i,j,k));
       end
